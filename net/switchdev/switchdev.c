				...
				struct netdev_phys_item_id *psid)
{
	...
	if (!get_ndo_ext(ops, ndo_switch_parent_id_get))
		return -EOPNOTSUPP;
	return get_ndo_ext(ops, ndo_switch_parent_id_get)(dev, psid);
}
...
int netdev_switch_port_stp_update(struct net_device *dev, u8 state)
{
	...
	if (!get_ndo_ext(ops, ndo_switch_port_stp_update))
		return -EOPNOTSUPP;
	WARN_ON(!get_ndo_ext(ops, ndo_switch_parent_id_get));
	return get_ndo_ext(ops, ndo_switch_port_stp_update)(dev, state);
}
...
static struct net_device *netdev_switch_get_lowest_dev(struct net_device *dev)
{
	...
	if (dev->features & NETIF_F_HW_SWITCH_OFFLOAD &&
	    get_ndo_ext(ops, ndo_switch_parent_id_get))
		return dev;
			       ...
			       u8 tos, u8 type, u32 nlflags, u32 tb_id)
{
	...
	if (get_ndo_ext(ops, ndo_switch_fib_ipv4_add)) {
		err = get_ndo_ext(ops, ndo_switch_fib_ipv4_add)(dev, htonl(dst),
								dst_len, fi,
								tos, type,
								nlflags, tb_id);
		if (!err)
			       ...
			       u8 tos, u8 type, u32 tb_id)
{
	...
	if (get_ndo_ext(ops, ndo_switch_fib_ipv4_del)) {
		err = get_ndo_ext(ops, ndo_switch_fib_ipv4_del)(dev, htonl(dst),
								dst_len, fi,
								tos, type,
								tb_id);
		if (!err)
